...
- **Stone blue palette + Calibri + logo top-right.** The Slate design tokens (Stone Blue accent `#6B8EA3`, Dark Slate `#2D3E4A`, Warm Gray `#8A8A8A`) are the Nightwork product palette. Calibri is the product typeface. Ross Built logo sits in the top-right of every authenticated surface. Hardcoded color hex codes, ad-hoc font-family declarations, and logo placement drift are all blocked by hooks.

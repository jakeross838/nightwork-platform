            ...
            <div className="mt-5 space-y-4">
              ...
              <div>
                ...
                <input value={vendorName} onChange={(e) => setVendorName(e.target.value)}
                  ...
                  className="w-full px-3 py-2.5 bg-brand-surface border border-teal/30 rounded-xl text-sm text-cream focus:border-teal focus:outline-none disabled:opacity-50" />
              </div>
              ...
              <div>
                ...
                <textarea value={qbNotes} onChange={(e) => setQbNotes(e.target.value)} rows={2} placeholder="Optional notes for QuickBooks entry..."
                  disabled={!isQaReviewable}
                  className="w-full px-3 py-2.5 bg-brand-surface border border-brand-border rounded-xl text-sm text-cream placeholder-cream-dim focus:border-teal focus:outline-none disabled:opacity-50 resize-none" />
              </div>
          ...
          <div className="bg-brand-card border border-brand-border rounded-2xl p-6 w-full max-w-md opacity-0 animate-fade-up shadow-2xl">
            ...
            <div className="flex gap-3 mt-4">
              <button onClick={handleKickBack} disabled={!kickBackNote.trim() || saving}
                className="flex-1 px-4 py-2.5 bg-status-danger text-white font-medium rounded-xl disabled:opacity-50 transition-all">
                Kick Back
              </button>
...
function LockedField({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div className="px-3 py-2.5 bg-brand-surface/50 border border-brand-border rounded-xl text-sm text-cream-muted" title="PM approved — kick back to change">
        {value}
      </div>
    </div>
  );

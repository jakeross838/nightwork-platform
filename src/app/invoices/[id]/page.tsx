...
function SearchCombobox({ label, value, onChange, options, disabled, aiFilled, grouped, placeholder }: {
 ...
 className="flex-1 bg-transparent text-slate-tile placeholder-cream-dim outline-none text-sm"
 onKeyDown={(e) => {
...
function AiBadge() {
 return <span className="inline-flex items-center px-1.5 py-0.5 text-[10px] font-bold bg-transparent text-stone-blue border border-stone-blue normal-case tracking-normal">AI</span>;
}
...
function LineCostCodeSelect({ value, onChange, options, disabled, aiSuggestion }: {
 ...
 {aiSuggestion && (
 <span className="absolute -top-2 right-1 text-[9px] px-1 bg-white text-stone-blue border border-stone-blue tracking-tight z-10">
 ...
 onMouseEnter={() => setHighlight(idx)}
 onClick={() => commit(o.value)}
 className={`w-full text-left px-2.5 py-1.5 text-xs transition-colors ${isHl ? "bg-slate-deep-muted" : ""} ${o.value === value ? "text-stone-blue font-medium" : "text-slate-tile"}`}>
 {o.label}
 </button>
 );
 })}
...
function statusDotColor(newStatus: string): string {
 ...
 if (["pm_held", "info_requested"].includes(newStatus)) return "bg-nw-warn";
 if (["pm_denied", "qa_kicked_back", "void"].includes(newStatus)) return "bg-nw-danger";
 return "bg-slate-deep"; // forward progress: pm_review, qa_review, ai_processed
}
...
export default function InvoiceReviewPage() {
 ...
 if (!invoice) return (
 <div className="min-h-screen flex items-center justify-center">
 <p className="text-nw-danger font-display text-lg">Invoice not found</p>
 </div>
 );
 ...
 {isCreditMemo && (
 <span className="inline-flex items-center text-xs px-3 py-1 font-medium bg-transparent text-stone-blue border border-stone-blue">
 Credit Memo
 </span>
 )}
 {isChangeOrder && (
 <span className="inline-flex items-center text-xs px-3 py-1 font-medium bg-transparent text-nw-warn border border-nw-warn">
 Change Order
 </span>
 )}
 ...
 {kickBackInfo && (
 <div className="bg-nw-danger-muted border-b border-nw-danger/20 px-6 py-3">
 <div className="max-w-[1600px] mx-auto flex items-start gap-3">
 <svg className="w-5 h-5 text-nw-danger flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
 <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126z" />
 </svg>
 <div>
 <p className="text-sm font-medium text-nw-danger">Kicked Back by Accounting</p>
 <p className="text-sm text-nw-danger/80 mt-0.5">{kickBackInfo}</p>
 </div>
 </div>
 </div>
 )}
 ...
 <p className="text-sm font-medium text-nw-danger">This invoice was denied</p>
 {denyInfo && <p className="text-sm text-nw-danger/80 mt-0.5">{denyInfo}</p>}
 <button
 ...
 <div>
 <p className="text-sm font-medium text-nw-danger">Math Mismatch Detected</p>
 <p className="text-sm text-nw-danger/80 mt-0.5">Line items may not sum to the stated total. Verify amounts before approving.</p>
 </div>
 </div>
 </div>
 )}
 ...
 <div className="mb-4 border border-nw-warn/50 bg-nw-warn/5 px-4 py-3 text-sm text-slate-tile animate-fade-up">
 <div className="flex items-start gap-3">
 <span className="inline-block px-2 py-0.5 text-[10px] uppercase tracking-wider border border-nw-warn text-nw-warn">
 Partial
 </span>
 <div className="flex-1">
 {invoice.parent_invoice_id ? (
 <p>
 <span className="font-medium text-slate-tile">Approved portion</span> of a split invoice.{" "}
 {siblingInvoice && (
 <Link href={`/invoices/${siblingInvoice.id}`} className="text-stone-blue hover:underline">
 ...
 <>
 <p>
 <span className="font-medium text-slate-tile">Held portion</span> — {siblingInvoice && (
 ...
 {siblingInvoice && (
 <Link href={`/invoices/${siblingInvoice.id}`} className="inline-block mt-1 text-stone-blue hover:underline">
 ...
 <div className="mb-4 border border-nw-warn/60 bg-nw-warn/10 px-4 py-3 text-sm animate-fade-up">
 <div className="flex items-start gap-3">
 ...
 .{" "}
 <Link href={`/invoices/${invoice.duplicate_of.id}`} className="text-stone-blue hover:underline font-medium">
 View existing &rarr;
 </Link>
 </p>
 </div>
 ...
 <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium bg-nw-danger-muted text-nw-danger border border-nw-danger/40">
 <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" /></svg>
 ...
 className={`relative inline-flex h-6 w-11 items-center transition-colors disabled:opacity-50 ${isChangeOrder ? "bg-nw-warn" : "bg-brand-border"}`}
 >
 ...
 <p className="mt-1.5 text-[11px] text-nw-warn">
 +{amountIncreasePct.toFixed(1)}% vs AI-parsed {formatCents(aiParsedTotal)}
 </p>
 )}
 {amountOver10Pct && (
 <p className="mt-1.5 text-[11px] text-nw-danger font-medium">
 Warning: +{amountIncreasePct.toFixed(1)}% over AI-parsed {formatCents(aiParsedTotal)} — note required
 </p>
 )}
 {isCreditMemo && (
 <span className="mt-1.5 inline-flex items-center px-2 py-0.5 text-[11px] font-medium bg-transparent text-stone-blue border border-stone-blue">
 Credit Memo
 </span>
 )}
 </div>
 ...
 <th className="py-2 px-3 text-left text-slate-tile font-semibold min-w-[120px]">CO Ref</th>
 ...
 className="text-[10px] text-stone-blue hover:underline mt-0.5"
 >
 ...
 disabled={!isReviewable}
 className={`relative inline-flex h-5 w-9 items-center transition-colors disabled:opacity-50 ${li.is_change_order ? "bg-nw-warn" : "bg-brand-border"}`}
 ...
 {hasAmountMismatch && (
 <div className="mt-2 px-3 py-2 bg-nw-danger-muted border border-nw-danger/20">
 <p className="text-xs text-nw-danger font-medium">
 ...
 : "bg-nw-success hover:brightness-110 disabled:opacity-50 disabled:hover:brightness-100"
 }`}>
 ...
 : "border-nw-success text-nw-success hover:bg-nw-success hover:text-white disabled:opacity-50 disabled:hover:bg-transparent disabled:hover:text-nw-success"
 }`}>
 Partial Approve
 </button>
 <button onClick={() => setShowNoteModal("hold")} disabled={saving}
 className="flex-1 px-4 py-3 bg-nw-warn hover:brightness-110 disabled:opacity-50 text-brand-bg font-medium transition-all">
 Hold
 </button>
 ...
 {budgetInfo.is_allowance && (
 <span className="inline-flex items-center px-1.5 py-0.5 text-[10px] font-bold bg-transparent text-stone-blue border border-stone-blue uppercase">Allowance</span>
 )}
 ...
 className="w-full px-4 py-2.5 bg-slate-deep hover:bg-slate-deeper text-brand-bg font-medium transition-colors disabled:opacity-50">
 ...
 <button onClick={() => setShowNoteModal("hold")} disabled={saving}
 className="flex-1 px-2 py-3 bg-nw-warn hover:brightness-110 disabled:opacity-50 text-brand-bg font-medium transition-all text-sm">
 Hold
 </button>
 <button onClick={() => setShowNoteModal("deny")} disabled={saving}
 className="flex-1 px-2 py-3 bg-nw-danger hover:brightness-110 disabled:opacity-50 text-white font-medium transition-all text-sm">
 Deny
 </button>
 </div>
 <button onClick={() => setShowRequestInfoModal(true)} disabled={saving}
 ...
 disabled={saving}
 className="flex-1 px-4 py-2.5 bg-nw-success hover:brightness-110 text-white font-medium disabled:opacity-50 transition-all">
 ...
 return (
 <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
 <div className="bg-white border border-nw-danger/40 p-6 w-full max-w-lg animate-fade-up shadow-2xl">
 <div className="flex items-center gap-3 mb-4">
 <div className={`w-10 h-10 flex items-center justify-center flex-shrink-0 ${hasRed ? "bg-nw-danger-muted" : "bg-nw-warn-muted"}`}>
 <svg className={`w-5 h-5 ${hasRed ? "text-nw-danger" : "text-nw-warn"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
 <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126z" />
 </svg>
 </div>
 <h3 className="font-display text-xl text-slate-tile">
 ...
 <span className={r.severity === "red" ? "text-nw-danger" : r.severity === "orange" ? "text-nw-warn" : "text-nw-warn"}>
 ...
 className="w-full px-4 py-2.5 bg-slate-deep hover:bg-slate-deeper text-brand-bg font-medium transition-all">
 ...
 className={`flex-1 px-4 py-2.5 font-medium disabled:opacity-50 transition-all ${hasRed ? "bg-nw-danger text-white" : "bg-nw-success text-white"} hover:brightness-110`}>
 ...
 {showAmountGuard && (
 <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
 <div className="bg-white border border-nw-danger/40 p-6 w-full max-w-md animate-fade-up shadow-2xl">
 <div className="flex items-center gap-3 mb-4">
 ...
 className="w-full px-4 py-2.5 bg-slate-deep hover:bg-slate-deeper text-brand-bg font-medium transition-colors">
 Go Back and Fill In
 </button>
 </div>
 </div>
 ...
 className="flex-1 px-4 py-2.5 bg-slate-deep hover:bg-slate-deeper text-brand-bg font-medium transition-colors">
 Go Back and Assign
 </button>
 <button
 onClick={() => { setShowMissingFieldsBlock(false); handleAction("hold", "Held — missing job or cost code assignment"); }}
 disabled={saving}
 className="flex-1 px-4 py-2.5 bg-nw-warn hover:brightness-110 text-brand-bg font-medium disabled:opacity-50 transition-all">
 Hold Instead
 </button>
 </div>
 </div>
 ...
 className={`flex-1 px-4 py-2.5 font-medium disabled:opacity-50 transition-all ${showNoteModal === "hold" ? "bg-nw-warn text-brand-bg" : "bg-nw-danger text-white"}`}>
 ...
 {partialError && (
 <div className="mt-3 border border-nw-danger/40 bg-nw-danger/5 px-4 py-2 text-sm text-nw-danger">
 {partialError}
 </div>
 )}
 ...
 className="flex-1 px-4 py-2.5 bg-nw-success hover:brightness-110 disabled:opacity-50 text-white font-medium transition-all"
 >
...
function OverBudgetAlert({ severity, overage, pct, isAllowance }: {
 ...
 yellow: { bg: "bg-nw-warn-muted", border: "border-nw-warn/30", text: "text-nw-warn", label: "Over budget" },
 orange: { bg: "bg-nw-warn-muted", border: "border-nw-warn/40", text: "text-nw-warn", label: "Significantly over budget" },
 red: { bg: "bg-nw-danger-muted", border: "border-nw-danger/40", text: "text-nw-danger", label: isAllowance ? "Allowance overage" : "Severely over budget" },
 };
...
function EditHistoryCard({
 ...
 {entries.map((entry, i) => (
 <div key={i} className="text-xs border-l-2 border-stone-blue/30 pl-3 py-1">
 <p className="text-slate-tile">
 <span className={`inline-flex items-center px-1.5 py-0.5 text-[10px] font-bold mr-1.5 bg-transparent border ${
 entry.source === "PM" ? "text-stone-blue border-stone-blue" : "text-nw-warn border-nw-warn"
 }`}>
 {entry.source}
 </span>
...
function PaymentSection({
 ...
 status === "paid"
 ? "border-nw-success text-nw-success"
 : status === "scheduled"
 ...
 {error && (
 <div className="mt-3 bg-nw-danger/10 border border-nw-danger/40 px-3 py-2 text-xs text-nw-danger">
 {error}
 </div>
 )}
 ...
 disabled={busy}
 className="px-3 py-2 border border-stone-blue text-stone-blue hover:bg-stone-blue/10 disabled:opacity-50 text-sm font-medium transition-colors"
 >
 ...
 disabled={busy}
 className="px-3 py-2 bg-slate-deep hover:bg-slate-deeper disabled:opacity-50 text-white text-sm font-medium transition-colors"
 >
 Mark as Paid
 </button>
 )}
 ...
 disabled={busy}
 className="px-3 py-2 border border-nw-danger text-nw-danger hover:bg-nw-danger/10 disabled:opacity-50 text-sm font-medium transition-colors"
 >
 Reverse Payment
 </button>
 )}

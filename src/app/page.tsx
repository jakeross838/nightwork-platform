...
export default function Home() {
 ...
 <div className="absolute top-1/4 -left-1/4 w-[600px] h-[600px] bg-teal/5 blur-[120px] pointer-events-none" />
 <div className="absolute bottom-1/4 -right-1/4 w-[500px] h-[500px] bg-brass/5 blur-[120px] pointer-events-none" />
...
function NavCard({ href, icon, color, title, subtitle, count }: {
 ...
 return (
 <Link href={href}
 className={`group relative flex flex-col items-start p-5 border border-brand-border bg-brand-surface/80 hover:bg-brand-elevated ${hoverBorder} transition-all duration-300 text-left`}>
 <div className={`flex items-center justify-center w-10 h-10 ${iconBg} mb-3 relative`}>
 <svg className={`w-5 h-5 ${iconColor}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>{icons[icon]}</svg>
 {count != null && count > 0 && (
 <span className="absolute -top-1.5 -right-1.5 flex items-center justify-center min-w-[20px] h-5 px-1.5 bg-brass text-brand-bg text-[11px] font-bold">{count}</span>
 )}
 </div>

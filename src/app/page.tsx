    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="px-6 pt-20 pb-24 bg-white-sand">
        <div className="max-w-[1100px] mx-auto text-center">
          <span className="inline-block px-3 py-1 border border-border-def text-[10px] tracking-[0.12em] uppercase text-tertiary mb-6">
          ...
          <div className="mt-10 flex items-center justify-center gap-4 flex-wrap">
            ...
            <Link
              href="/pricing"
              className="px-6 py-3 border border-border-def text-slate-tile text-[13px] tracking-[0.12em] uppercase font-medium hover:bg-bg-sub transition-colors"
            >
              See Pricing
            </Link>
          </div>
          <p className="mt-4 text-xs text-tertiary">No credit card required · 14-day free trial</p>
        </div>
      </section>

      {/* Features */}
      <section className="px-6 py-20 bg-white border-y border-border-def">
        <div className="max-w-[1100px] mx-auto">
          <div className="text-center mb-16">
            <span className="text-[10px] tracking-[0.12em] uppercase text-tertiary">Capabilities</span>
      ...
      <section className="px-6 py-20">
        <div className="max-w-[1100px] mx-auto">
          <div className="text-center mb-16">
            <span className="text-[10px] tracking-[0.12em] uppercase text-tertiary">Workflow</span>
            <h2 className="mt-2 font-display text-3xl md:text-4xl text-slate-tile tracking-tight">
      ...
      {/* Pricing preview */}
      <section className="px-6 py-20 bg-bg-sub border-t border-border-def">
        <div className="max-w-[1100px] mx-auto">
          <div className="text-center mb-12">
            <span className="text-[10px] tracking-[0.12em] uppercase text-tertiary">Pricing</span>
            <h2 className="mt-2 font-display text-3xl md:text-4xl text-slate-tile tracking-tight">
...
function FeatureCard({ title, body }: { title: string; body: string }) {
  return (
    <div className="p-6 border border-border-def bg-white">
      <h3 className="font-display text-lg text-slate-tile">{title}</h3>
      <p className="mt-3 text-sm text-secondary leading-relaxed">{body}</p>
    </div>
  );
}
...
function PricePreview({ name, price, tagline, highlight }: { name: string; price: string; tagline: string; highlight?: boolean }) {
  return (
    <div className={`p-6 border bg-white ${highlight ? "border-stone-blue shadow-[0_8px_24px_-12px_rgba(63,88,98,0.25)]" : "border-border-def"}`}>
      {highlight && (
      ...
      <p className="mt-4 font-display text-4xl text-slate-tile">
        {price}
        <span className="text-sm text-tertiary font-sans">/mo</span>
      </p>

...
export function confidenceColor(score: number): string {
 if (score >= 0.85) return "bg-transparent text-nw-success border border-nw-success";
 if (score >= 0.7) return "bg-transparent text-nw-warn border border-nw-warn";
 return "bg-transparent text-nw-danger border border-nw-danger";
}
...
export function statusBadgeOutline(status: string): string {
 if (["pm_approved", "qa_approved", "pushed_to_qb"].includes(status)) return "bg-transparent text-nw-success border border-nw-success";
 if (["in_draw", "paid"].includes(status)) return "bg-transparent text-[rgba(59,88,100,0.55)] border-[rgba(59,88,100,0.30)]";
 if (["pm_review", "ai_processed", "qa_review", "received", "info_requested"].includes(status)) return "bg-transparent text-nw-warn border border-nw-warn";
 if (["pm_held"].includes(status)) return "bg-transparent text-nw-warn border border-nw-warn";
 if (["qa_kicked_back", "pm_denied", "void", "qb_failed"].includes(status)) return "bg-transparent text-nw-danger border border-nw-danger";
 return "bg-transparent text-[rgba(59,88,100,0.55)] border border-[rgba(59,88,100,0.30)]";
}

...
export function SkeletonStatCard() {
  return (
    <div className="flex flex-col items-start p-4 border border-border-def bg-white">
...
export function SkeletonTableRow({ columns }: { columns: string[] }) {
  return (
    <div className="grid items-center gap-4 px-4 py-3 border-b border-border-def" style={{ gridTemplateColumns: columns.map(() => "1fr").join(" ") }}>
      {columns.map((width, i) => (
...
export function SkeletonList({ rows = 5, columns }: { rows?: number; columns: string[] }) {
  return (
    <div className="border border-border-def bg-white" aria-busy="true" aria-label="Loading">
...
export function SkeletonCard({ height = "h-32" }: { height?: string }) {
  return (
    <div className={`p-5 border border-border-def bg-white ${height}`}>
...
export function SkeletonBlock({ children, height = "h-48" }: { children?: ReactNode; height?: string }) {
  return (
    <div className={`p-4 border border-border-def bg-white ${height}`} aria-busy="true">
      {children ?? (
        <>

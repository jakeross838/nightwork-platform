  ...
  if (!editing) {
    return (
      <button
        ...
        } hover:bg-[rgba(91,134,153,0.08)] px-1 py-1 sm:py-0 rounded-sm transition-colors cursor-text`}
      ...
      >
        ...
        <span className="absolute right-1 top-1/2 -translate-y-1/2 opacity-100 md:opacity-0 md:group-hover:opacity-100 text-[color:var(--text-secondary)] pointer-events-none">
  ...
  return (
    <span className={`inline-flex flex-col ${alignRight ? "items-end" : "items-start"} w-full`}>
      <input
        ...
        className={`px-1 py-0.5 text-[13px] bg-[var(--bg-subtle)] border border-[var(--nw-stone-blue)] text-[color:var(--text-primary)] focus:outline-none focus:ring-1 focus:ring-teal tabular-nums ${
          alignRight ? "text-right" : "text-left"
        } w-full`}
      />
      {error && <span className="text-[10px] text-[color:var(--nw-danger)] mt-0.5">{error}</span>}
    </span>
  );
}

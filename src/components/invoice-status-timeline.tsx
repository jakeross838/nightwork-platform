  ...
  return (
    <div className="border border-border-def bg-white p-4">
      ...
      <div className="hidden md:block">
        <div className="flex items-start relative pt-3">
          {stages.map((stage, i) => (
            <div key={stage.key} className="flex-1 flex flex-col items-center relative min-w-0">
                ...
                <div
                  className={`absolute top-[11px] left-0 right-1/2 h-px ${
                    stage.isPast || stage.isCurrent
                      ? "bg-slate-deep"
                      : "border-t border-dashed border-border-def"
                  }`}
                  style={
                    !stage.isPast && !stage.isCurrent
              ...
              <div className="mt-2 text-[10px] text-center text-tertiary leading-tight px-1">
                {stage.label}
                {stage.entry?.when ? (
                  <div className="mt-0.5 text-[9px] text-tertiary/70">
        ...
        {/* Branch indicators */}
        {branches.length > 0 && (
          <div className="mt-4 pt-3 border-t border-border-def flex flex-wrap gap-2">
            {branches.map((b, i) => {
      ...
      <div className="md:hidden">
          ...
          {stages.map((stage, i) => (
            <div key={stage.key} className="relative pl-6 pb-3 last:pb-0">
              {i < stages.length - 1 && (
                <div
                  className={`absolute left-[7px] top-4 bottom-0 w-px ${
                    stage.isPast ? "bg-slate-deep" : "border-l border-dashed border-border-def"
                  }`}
                  style={
                    !stage.isPast
              ...
              <div className="text-xs">
                <p className={`font-medium ${stage.isCurrent ? "text-slate-tile" : stage.isPast ? "text-secondary" : "text-tertiary"}`}>
                  {stage.label}
                </p>
                {stage.entry?.when ? (
                ...
                {stage.entry?.note ? (
                  <p className="text-tertiary/80 mt-1 italic text-[10px] leading-relaxed">
        ...
        {branches.length > 0 && (
          <div className="mt-3 pt-3 border-t border-border-def space-y-1.5">
            {branches.map((b, i) => {
              const ns = String(b.new_status ?? "");
              const color = BRANCH_COLORS[ns] ?? "bg-brand-border";
              return (
...
function TimelineDot({
  ...
  const fill = stage.isCurrent
    ...
    : stage.isPast
      ...
      : "bg-transparent border-2 border-border-def";
  return (
    <div
      className="relative"
    ...
    >
      ...
      {hovered && stage.entry && (
        <div className="absolute left-1/2 -translate-x-1/2 top-full mt-2 z-30 bg-white-sand/95 border border-border-def p-2 text-[11px] text-slate-tile min-w-[180px] shadow-lg pointer-events-none">
          ...
          {stage.entry.note ? (
            <p className="text-tertiary/80 mt-1 italic leading-relaxed">
              {String(stage.entry.note)}
            </p>
          ) : null}
        </div>

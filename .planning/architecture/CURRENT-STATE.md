...
|---|---|---|
| Drummond construction site address | P-drive folder "501 74th St" + Downloads filenames `*501*74*` (10 hits) | Construction address is **501 74th, Holmes Beach FL** — corroborated by P-drive and Downloads (per Jake 2026-04-29). |
| Supabase `jobs.address` and `client_name` for Drummond | Supabase Source 2 record | ⚠ **SYNTHETIC** — placeholder address ("712 North Shore Dr, Anna Maria, FL") and placeholder client name in the Supabase Drummond row are **synthetic test data, not real** (per Jake 2026-04-29). The real homeowners and addresses are intentionally not committed to git. |
...
**Address resolution (Jake 2026-04-29):** Real Drummond construction site is **501 74th, Holmes Beach FL**. The Supabase Drummond record's address ("712 North Shore Dr, Anna Maria, FL") and client name ("Kevin and Robin Drummond") are placeholder/synthetic test data — not the real homeowners. The real Drummond data has not been ingested into Supabase. PM is **Bob Mozine** per CLAUDE.md PM list.

This makes the data-ingestion gap finding much stronger: the Supabase Drummond record is a synthetic stub; **all real Drummond financial history exists only in P-drive (folders) and Downloads (files)**. Jake's clarification reframes the "address divergence" finding: it isn't a Ross Built convention — it's evidence that Supabase Drummond data is synthetic.

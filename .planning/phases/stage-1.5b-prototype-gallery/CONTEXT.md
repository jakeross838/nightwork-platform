...
- **D-29 — Cloud-sync vector confirmed closed (per nwrp34 Part 5).** Jake confirmed 2026-05-04: `C:\Users\Jake\` filesystem path is **NOT** cloud-synced (no OneDrive / Dropbox / iCloud Drive sync target). Raw Drummond fixtures in `.planning/fixtures/drummond/` (gitignored) exist only locally on Jake's PCs + git-clone-on-Jake's-machine. T-1.5b-W0-06 (cloud-sync leak vector audit) closed. Future PCs must verify same posture before staging raw fixtures locally.
- **D-30 — Jeff Bryde Drummond PM-of-record audit (per nwrp34 P3).** Cross-referenced raw Drummond fixtures (`.planning/fixtures/drummond/source3-downloads/` — 13 priority files including 5 pay apps XLSX, budget XLSX, schedule XLSX, lien releases PDF, combined invoices PDF, contract DOCX, 6 split-invoices) for "Jeff Bryde" / "Bryde" via case-insensitive grep + binary string scan via `strings`. **0 hits.** Conclusion: Jeff Bryde is **NOT** a Drummond PM-of-record per the inspected raw fixture set. Exclusion from the 32-entry CI / pre-commit / extractor denylist is correct. The 5 PMs in the denylist (Lee Worthy / Nelson Belanger / Bob Mozine / Jason Szykulski / Martin Mannix) reflect Jake's nwrp31 #3 explicit list of canonical PMs; Jeff Bryde is on the broader CLAUDE.md PM roster but not active on Drummond per fixture inspection.
- **D-31 — M3 phone gate LOCKED (per nwrp34 Part 4).** Real-phone test device: **iPhone on Safari** (current iPhone Jake has on hand; whatever current Safari version ships with iOS at walkthrough time). Gate is "Jake walks every prototype on his actual phone" — not version-specific compliance. EXPANDED-SCOPE.md §0 + MANUAL-CHECKLIST.md M3 + SETUP-COMPLETE.md substituted. Concrete enough for QA spec-checker validation.
- **D-32 — Wave 0 fixture coverage gaps + closures (per nwrp38, locked 2026-05-04).** Wave 0 first-pass produced 6 invoices (2 progress + 4 lump_sum) and 3 PCCOs from the 6 split-invoice PDFs in `source3-downloads/`. Three gaps surfaced at HALT review:
  1. **T&M format absent** — None of the 6 split-invoice PDFs have crew × hours × rate structure. CLOSED via nwrp38 Option B: 1 synthesized T&M invoice added (Bay Region Carpentry Inc, framing labor, 5 daily entries, $8,377 total, status `pm_review`, marked `synthesized` in flags + `[SYNTHESIZED per nwrp38]` description prefix). All 3 invoice formats now present in fixtures (progress + lump_sum + time_and_materials).
  2. **Change order count 3 vs 4-6 target** — CLOSED. 2 synthesized PCCOs added: PCCO #4 framing scope reduction (DEDUCTIVE, -$8,500, Bay Region Carpentry, 2025-11-08); PCCO #5 drywall allowance reconciliation (ALLOWANCE_RECON, +$4,400, Sandhill Drywall, 2025-12-10). Total now 5 PCCOs covering ≥1 additive + ≥1 deductive + ≥1 allowance reconciliation per nwrp38 spec. Both new PCCOs marked `[SYNTHESIZED per nwrp38]` in description prefix.
  3. **Lien release statutes 2 of 4** — `conditional_progress` + `unconditional_progress` only. ACCEPTED per nwrp38 Option A (no fixture additions). Caldwell is mid-construction (synthetic Substantial Completion 2026-03-15); `conditional_final` + `unconditional_final` would require fabricated Substantial Completion data, which crosses the synthesis threshold beyond what nwrp38 authorized for COs/T&M. Real final-stage releases will surface in Wave 1.1 customer use after first-tenant Substantial Completion event. The 3 lien releases in fixtures (SmartShield/Loftin/Paradise Foam unconditional_progress + conditional_progress) demonstrate the workflow shape; final-stage variants are deferred.

  Synthesized records traceability: Bay Region Carpentry T&M invoice + PCCO #4 + PCCO #5 are tagged via (a) `synthesized` token in flags array (invoices) or (b) `[SYNTHESIZED per nwrp38]` description prefix (change_orders). Wave 2 Plan 6's `findings.md` aggregator (per EXPANDED-SCOPE §7 ac12) scans for these markers and lists synthesized records explicitly so QA spec-checker can distinguish raw-derived from fabricated fixtures. The hand-curated source `scripts/drummond-invoice-fields.json` `_meta.synthesized_records_note` documents the synthesis rationale; the JSON itself is gitignored (verified .gitignore:87).
